# AskElira — Claude Code Plugin

Multi-agent autonomous building pipeline for Claude Code. Describe a goal, AskElira decomposes it into floors and ships real working code. Inspired by [AskElira/AskElira3](https://github.com/AskElira/AskElira3) but reimplemented as native Claude Code sub-agents — no separate Hermes runtime, no Node infrastructure, no SQLite. The Claude Agent SDK is the runtime.
...
```
/askelira:build a REST API for a todo app with SQLite, JWT auth, and tests
```
...
```
/plugin marketplace add AskElira/AskElira3
/plugin install askelira@askelira-marketplace
```

### Method 2 — Clone into your project

```bash
git clone https://github.com/AskElira/AskElira3 .claude/plugins/askelira
```
...
Drop the cloned folder into `~/.claude/plugins/askelira/` to use across all your projects.

## Use it

```
/askelira:build a Python CLI that scrapes Hacker News top stories and emails them daily
```

```
/askelira:plan a Next.js dashboard with Supabase auth and Stripe billing
```

```
/askelira:fix workspaces/my-goal-slug 3 "the API returns 500 on /todos POST"
```
...
├── commands/
│   ├── build.md             # /askelira:build — full pipeline
│   ├── plan.md              # /askelira:plan — preview floors
│   ├── fix.md               # /askelira:fix — manual Steven
│   └── status.md            # /askelira:status — list workspaces
├── skills/
│   └── askelira-build/
...
| AskElira3 (Node app)                  | AskElira (Claude plugin)                 |
|---------------------------------------|------------------------------------------|
...
| `npm start && askelira3 "<goal>"`     | `/askelira:build <goal>`                 |
...
Inspired by [AskElira/AskElira3](https://github.com/AskElira/AskElira3) and [forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills).
